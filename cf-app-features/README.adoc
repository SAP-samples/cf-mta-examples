...
NOTE: See link:mta.yaml[mta.yaml] or link:mtad.yaml[mtad.yaml] for details on the `app-features` parameter configuration. Extension descriptor examples are available: link:development.mtaext[development.mtaext], link:production.mtaext[production.mtaext], and link:security.mtaext[security.mtaext].
