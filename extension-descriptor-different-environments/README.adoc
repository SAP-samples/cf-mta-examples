...
The MTA will be deployed in DEV and PROD environments using different extension descriptors resulting in different setups.
...
That approach uses deployment descriptor `link:mtad.yaml[mtad.yaml]`, ready application binaries `link:appBits.zip[appBits.zip]` and extension descriptor for DEV env `link:dev.mtaext[dev.mtaext]`:
