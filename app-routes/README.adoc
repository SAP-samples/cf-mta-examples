...
### Deploy
There are two ways of deploying the MTA. The first one is directly from an `mtad.yaml` deployment descritpr:
```bash
  $ cf deploy ./ -f ;
```
Alternatively, you can build it from an `mta.yaml` development descriptor and deploy the `mtar` archive afterwards:
```bash
  $ mbt build -p cf -t . ;
