...
| `port`    | The app listens on the port CF assigns (`$PORT`) but does not serve HTTP, or you don't want to expose a health endpoint. Applied by default when `health-check-type` is not set.
