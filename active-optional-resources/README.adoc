...
This example shows how to mark service resources as optional/mandatory or active/inactive in order to influence the deployment behavior.
...
If a service instance is not mandatory for a deployment but you want it to be used if available, it can be marked as `optional` in the `mta(d).yaml` descriptor:
