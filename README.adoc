...
Most of the examples are generic for link:https://github.com/cloudfoundry-incubator/multiapps-controller[MultiApps Controller] that is the standard MTA Deployment service for Cloud Foundry. However, some of the examples are designated only for SAP Cloud Platform Cloud Foundry, not for other distributions of Cloud Foundry, because of offerings and other functionalities available only there.
