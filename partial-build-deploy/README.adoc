...
You can find a few tips about usage of the Cloud MTA Builder commands for the partial build in the link:https://github.com/SAP-samples/cf-mta-examples/tree/main/partial-build-deploy#build-only-specific-mta-modules-defined-in-the-mtayaml-file-and-deploy-them[Build only specific MTA modules defined in the `mta.yaml` file and deploy them] section below.   
